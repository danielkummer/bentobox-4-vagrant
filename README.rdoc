...
=== Capistrano ngix deployment

Bentobox is configured for capistrano ngix deployment, the following additional capistrano tasks are available:

    cap nginx:setup
    cap nginx:reload

They are automatically invoked with cap deploy:setup.
